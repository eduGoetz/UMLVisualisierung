...
fn main() {
	...
	//anfangswerte erst = 100 zweit = 250
	...
	while !fertig { 	
		...
		let mut ueberschrift = Scale { x: 15.0 * 2.0, y: 15.0 };
		
		ab=erster_wert;
		//beschriftung vom bild
		let mut done_schrift = false; 
		let mut zahl = 1;
		draw_text_mut(&mut image, Rgb([0u8, 0u8, 0u8]), erster_wert_x, ab+5, ueberschrift, &font,  "Überschrift");
		let mut attribute = Scale { x: 10.0, y: 10.0 };;
		
		ab=50;
		
		while !done_schrift {
			draw_text_mut(&mut image, Rgb([0u8, 0u8, 0u8]), erster_wert_x+10, ab, attribute, &font,  "hallo");
